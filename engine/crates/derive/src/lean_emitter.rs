...
pub fn emit_lean_file(ctx: &DerivationContext, config: &LeanEmitConfig) -> String {
    ...
    // ── Header comment ───────────────────────────────────────
    // Important: deliberately do NOT include `Utc::now()` here. Lake
    // hashes the full source string to decide whether a cached .olean
    // is still valid. A per-invocation timestamp makes every emit
    // unique → cache miss → cold ~30-60 min recompile on every chain
    // verification (Mathlib + LeafImports + Basic + Derived). Removing
    // the timestamp lets identical chains hit the cache and finish in
    // ~1-2 seconds. The chain semantics already determine the source
    // bytes; the audit trail of "when did we generate this" belongs in
    // the DB row, not the .lean file.
    out.push_str(&format!(
    ...
    // until memory exhaustion. The first try (maxHeartbeats=200000,
    // Mathlib's default) was still letting bad candidates eat 50+ min
    // wall on the swap-bound droplet — the 4-tactic `first` cascade
    // each got its own 200k budget so the total was effectively 800k.
    // 50000 is aggressive but the M1.c seed chain closed in well under
    // that limit on the same hardware, so the known-good path doesn't
    // regress. Any legitimate physics chain we care about should close
    // in <50k heartbeats; anything slower than that is GA noise.
    out.push_str("set_option maxHeartbeats 50000\n\n");
    // ── Namespace + opens ────────────────────────────────────
