...
# ──────────────────────────────────────────────────────────────────────────
# Build stage: cargo build --release of physics-api + worker +
...
# ──────────────────────────────────────────────────────────────────────────
# Worker target: same as runtime + Lean toolchain (elan) + worker
# Used by the `ga-worker` service in docker-compose.
# ──────────────────────────────────────────────────────────────────────────
FROM runtime AS worker

COPY --from=builder /src/engine/target/release/worker /usr/local/bin/worker
...
CMD ["/usr/local/bin/worker", "--max-lake=15"]
